...
*   [][][]
*   [][]
- Programação Paralela e Distribuida - 2022.2 - Prof.Cidcley                                                   
...
import paho.mqtt.client as mqtt  #(MOM)
...
endereco_Broker = "test.mosquitto.org"  #broker quelquer pra teste

# As estruturas "Queue" são melhores para se guardar as strings de msgs lidas nos tópicos pois
# são melhores para estruturas com threads (Queues são 'thread safe')!
...
# Links que ajudaram na implementação e entendimento dessas duas funções 'on_message' e 
# como usá-las nesse código:
...
def janela_Cria_Cliente():
    ...
    client_subscriber_Recebe_Topicos_Sensores = mqtt.Client("recebe_lista_Topicos_Sensores") # (MOM)
    client_subscriber_Recebe_Topicos_Sensores.connect(endereco_Broker)                       # (MOM)
    client_subscriber_Recebe_Topicos_Sensores.subscribe("mostra_Topicos/Sensores")           # (MOM)
    client_subscriber_Recebe_Topicos_Sensores.loop_start()                                   # (MOM) - Loop
    ...
    newWindow.title("MOM: Cliente - T3ppd - icarOS")
    ...
    bg_label = Label(newWindow,image = img_janela_Inicial_cliente_bg_asset, width=400, height=350)
    ...
    lbl_with_my_gif.config(bg='#34ff59')
    lbl_with_my_gif.place(x=135, y=109)
    ...
    cria_cliente_button.place(x=124, y=237)
...
def janela_Principal_Cliente(newWindow_close,nome_Cliente,topicos):
    ...
    # MQTT - Cliente
    client_subscriber_Cliente = mqtt.Client(nome_Cliente) # (MOM)
    client_subscriber_Cliente.connect(endereco_Broker) # (MOM)
    ...
    for i in range(len(array_topicos)):
        client_subscriber_Cliente.subscribe(array_topicos[i]) # (MOM)
    ...
    bg_label_nome_cliente.config(bg='#34ff59')
    ...
    client_subscriber_Cliente.loop_start() #  (MOM)
    client_subscriber_Cliente.on_message=on_message #  (MOM)
